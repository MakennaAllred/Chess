...
public class ServerFacadeTests {
    ...
    @BeforeEach
    public void clear(){
        auth = new ServerFacade(port, null).register(new UserData("z","b","c"));
        new ServerFacade(port, null).deleteAll(auth.authToken());
    }
    ...
    public void registerTestPos() throws BadRequestException, DataAccessException, AlreadyTakenException {
        auth =  new ServerFacade(port, null).register(user);
    ...
    public void registerTestNeg() {
        // empty fields
        assertThrows(RuntimeException.class, () -> new ServerFacade(port, null).register(new UserData(null,"pass","e@mail.com")));
    }

    @Test
    public void loginTestPos() throws UnauthorizedException, DataAccessException, BadRequestException, AlreadyTakenException {
        auth =  new ServerFacade(port, null).register(user);
        auth = new ServerFacade(port, null).login(user);
        assertNotNull(auth.authToken());
    }
    ...
    public void loginTestNeg(){
        ...
        assertThrows(RuntimeException.class, () -> new ServerFacade(port, null).login(new UserData("a", "b", "c")));
    }
    ...
    public void logoutTestPos() throws UnauthorizedException, DataAccessException, BadRequestException, AlreadyTakenException {
        auth =  new ServerFacade(port, null).register(user);
        auth = new ServerFacade(port, null).login(user);
        assertDoesNotThrow(() -> new ServerFacade(port, null).logout(auth.authToken()));
    }

    @Test
    public void logoutTestNeg(){
        // no auth token
        assertThrows(RuntimeException.class,() -> new ServerFacade(port, null).logout(null));
    }
    ...
    public void createGameNeg() throws UnauthorizedException, BadRequestException, DataAccessException {
        //didn't log in
        assertThrows(RuntimeException.class, () ->new ServerFacade(port, null).createGame("1234",new GameData(1,null,null, "fakeGame", new ChessGame())));

    }
    @Test
    public void createGamePost() throws BadRequestException, DataAccessException, AlreadyTakenException, UnauthorizedException {
        AuthData auth = new ServerFacade(port, null).register(user);
        CreateGameRes game = new ServerFacade(port, null).createGame(auth.authToken(),new GameData(1,null,null, "fakeGame", new ChessGame()));
    ...
    public void listGamesPos() throws BadRequestException, DataAccessException, AlreadyTakenException, UnauthorizedException {
        ...
        auth = new ServerFacade(port, null).login(user);
        CreateGameRes game = new ServerFacade(port, null).createGame(auth.authToken(),new GameData(1,null,null, "fakeGame", new ChessGame()));
        ListGamesRes games =  new ServerFacade(port, null).listGames(auth.authToken());
        assertEquals(1,games.games().size());
    }

    @Test
    public void listGamesNeg(){
        assertThrows(RuntimeException.class, () -> new ServerFacade(port, null).listGames("1234"));
    }
    ...
    public void joinGamePos() throws BadRequestException, DataAccessException, AlreadyTakenException, UnauthorizedException {
        ...
        auth = new ServerFacade(port, null).login(user);
        CreateGameRes game = new ServerFacade(port, null).createGame(auth.authToken(),new GameData(1,null,null, "fakeGame", new ChessGame()));
        new ServerFacade(port, null).joinGame(auth.authToken(),new JoinGameReq("WHITE", 1));
    ...
    public void joinGameNeg() throws UnauthorizedException, BadRequestException, DataAccessException, AlreadyTakenException {
        ...
        auth = new ServerFacade(port, null).login(user);
        CreateGameRes game = new ServerFacade(port, null).createGame(auth.authToken(),new GameData(1,null,null, "fakeGame", new ChessGame()));
        assertThrows(RuntimeException.class, () -> new ServerFacade(port, null).joinGame(auth.authToken(), new JoinGameReq("WHITE", 0)));
    }
    @Test
